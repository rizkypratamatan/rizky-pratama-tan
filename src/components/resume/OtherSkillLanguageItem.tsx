...
export default async function OtherSkillLanguageItem({image, name, percentage}: Props) {
            ...
            <div className="flex gap-3 md:gap-5 justify-between items-center">
                <div className="basis-30 xs:basis-60 flex justify-between min-w-40">
                    {Array.from({length: 10}).map((_, index: number) =>
                        <span key={index} className={`${(index + 1) * 10 <= percentage ? 'bg-primary-100' : 'bg-border-100'} size-2.5 rounded-full xs:size-4`}></span>)}
                </div>
